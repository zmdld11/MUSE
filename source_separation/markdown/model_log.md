...
**模型结构**:
```
Input: [B, 1, 66150] → Conv1D×5 (k=8 s=4 GLU: 1→16→32→64→128→256)
→ BLSTM(256→512) → ConvTranspose1D×5 + skip → [B, 1, 66150]
```
参数量: **2,971,622** | Loss: 波形 L1

**训练 (5 epoch)**: L1 0.127→0.040, SDR -0.38 dB 时域 | 每 epoch ~8s

**下一步**: 加 MoisesDB + 100 epoch
