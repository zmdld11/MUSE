/** GM program → gleitz/midi-js-soundfonts 音色名（smplr FluidR3_GM 套件） */
export const GM_GLEITZ_NAMES: string[] = [
...
/** 鼓轨（channel 10）暂无 GM 鼓组采样，用 synth_drum 兜底（M3+ 换完整鼓组） */
export function gleitzInstrumentName(program: number, isDrum = false): string {
  if (isDrum) return "synth_drum";
  return GM_GLEITZ_NAMES[program] ?? "acoustic_grand_piano";
}
