...
class OnsetsAndFrames(nn.Module):
    def __init__(self, n_mels=229, n_midi=88, include_offset=False):
        super().__init__()
        self.n_midi = n_midi
        ...
        if include_offset:
            ...
            self.offset_head = nn.Sequential(
                ...
                nn.Linear(128, n_midi), nn.Sigmoid())
    ...
    def forward(self, spec):
        ...
        out = {"onset": onset, "frame": frame}
        if self.include_offset:
            x_prev = torch.cat([x[:, :1, :], x[:, :-1, :]], dim=1)
            out["offset"] = self.offset_head(torch.cat([x, x - x_prev], dim=-1))
        return out
