...
**逐类最优阈值 (从 VER3.1 最佳模型 + 验证集计算)**：

| 乐器 | 最优阈值 | 验证 F1 (at threshold) |
|------|----------|------------------------|
| acoustic guitar | 0.50 | 0.9812 |
| cello | 0.50 | 0.9532 |
| drum set | 0.35 | 0.9821 |
| electric bass | 0.35 | 0.9453 |
| electric guitar | 0.30 | 0.9674 |
| flute | 0.40 | 0.9371 |
| piano | 0.30 | 0.9344 |
| **singer** | **0.35** | **0.9758** |
| synthesizer | 0.55 | 0.9485 |
| violin | 0.35 | 0.9301 |

**关键发现**：
- **singer 的最优阈值仅 0.35，以前全局 0.65 几乎压死了所有歌手检测**
- 所有类别的最优阈值（0.30-0.55）均显著低于之前的全局 0.65
- 之前的"幻觉抑制"策略（提高全局阈值）过度牺牲了召回率，是导致漏检的根因
- 高阈值对常见类（电贝斯、钢琴影响小）稀有类（singer、合成器影响大）的伤害不对称
- 背景样本（8%全零标签）已经在训练中教会了模型"没人"的概念，不需要靠高阈值抑制幻觉

**评估**：
- 不改模型结构，仅改变推理判定逻辑
- 不影响训练流程（模型权重完全复用 VER3.1）
- 逐类最优阈值在验证集上给出近乎完美的各类别 F1（>93%），验证了模型本身的能力不差，问题出在推理阈值
